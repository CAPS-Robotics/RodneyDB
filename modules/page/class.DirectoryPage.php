...
'<script src="assets/js/directory.js"></script>';
    ...
    public function getDirectoryTable($rank) {
        ...
        foreach ($teamArr as $member) {
            $tableStr .= "<tr id=\"". $member['id'] ."\"><td>" . ( $rank > 7 ? ($member['rank'] == 5 ? "<a href='?p=confirm&id=" . $member['id'] . "' title='Confirm user'>" : "") : "" ) . $member['name'] . ( $rank > 7 ? ($member['rank'] == 5 ? "</a>" : "") : "" ) . " <span class='label label-" . ($member['rank'] == 7 ? "success"  : ($member['rank'] == 8 ? "danger" : ($member['rank'] == 9 ? "warning" : ($member['rank']>= 10 ? "primary" : "default" ) ) ) ) . "'>" . Utils::getRankName($member['rank']) . "</span></td><td>".($rank>7?"<span class='editable'/>":"") . $member['email'] . "</td><td>".($rank>7?"<span class='editable'/>":"") . Utils::formatPhoneNum($member['phone']) . "</td>" . ($rank>= 7 ? "<td>".($rank>7?"<span class='editable'/>":"") . $member['studentId'] . "</td>" . "<td>".($rank>7?"<span class='editable'/>":"") . $member['hours'] . "</td>" . ($rank>= 10 ? "<td><a href='?p=del&id=" . $member['id'] . "'>Delete</a></td>" : "") : "") . "</tr>";
        }
        return $tableStr;
    }
