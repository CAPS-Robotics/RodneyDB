...
class xhttp {
    ...
    public static function request($url, $method = 'get', $requestData = array()) {
        ...
        if(isset($requestData['get'])     and $requestData['get'] ) $urlparts['query'] = self::toQueryString($requestData['get']);
        if(isset($requestData['post'])    and $requestData['post']) $requestData['curl'][CURLOPT_POSTFIELDS] =& $requestData['post'];
        if(isset($requestData['headers']) and $requestData['headers']) $requestData['curl'][CURLOPT_HTTPHEADER] = self::toCurlHeaders($requestData['headers']);
        if(isset($requestData['cookies']) and $requestData['cookies']) $requestData['curl'][CURLOPT_COOKIE]  = self::toCookieString($requestData['cookies']);
        ...
        } elseif((isset($requestData['curl'][CURLOPT_POSTFIELDS]) && is_array($requestData['curl'][CURLOPT_POSTFIELDS])) and (!isset($requestData['headers']['Content-Type']) or $requestData['headers']['Content-Type'] == 'application/x-www-form-urlencoded')) {
    ...
    public static function cookie_decode($cookies, $urldecode = false) {
        ...
        foreach($cookies as $cookie) {
            ...
            foreach($crumbles as $crumble) {
                //list($name, $value) = explode('=', $crumble, 2);
				$exploded = explode('=', $crumble, 2);
    ...
    public static function toQueryString(&$array, $urlencode = true) {
        ...
        if(is_array($array)) foreach($array as $key => $value)
            $string .= $key.'='.(($urlencode) ? urlencode($value): $value).'&';
        return rtrim($string, '&');
    }
